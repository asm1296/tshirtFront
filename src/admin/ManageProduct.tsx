import React,{useState, useEffect} from 'react';
import Base from '../core/Base';
import { Link } from 'react-router-dom';
import { deleteProduct, getAllProducts } from './helper/adminapicall';
import { isAuthenticate } from '../auth/helper';

type product = {
    _id:string,
    name:string
}

const ManageProduct = ()=>{

    const [products, setProducts] = useState([])

    const {user, token} = isAuthenticate();

    const preloadData =()=>{
        getAllProducts().then(data=>{
            if(data.error){
                console.log(data.error)
            }
            else{
                setProducts(data)
            }
        })

    }

    useEffect(()=>{
        preloadData()
    },[])

    const deleteThisProduct = (productId:string) =>{
        deleteProduct(productId,user._id,token).then(data=>{
            if(data.error){
                console.log(data.error)
            }
            else{
                preloadData();
            }
        })
    }

    return(
        <Base title="Welcome admin" description="Manage products here">
            <div>
                <h2 className="text-white mb-3">All Products</h2>
                <Link className="btn btn-warning" to={`/admin/dashboard`}>
                    <span className="">Admin Home</span>
                </Link>
                <div className="row">
                    <div className="col-12">
                        <h2 className="text-center text-white my-3">Total {products.length} products</h2>
                    </div>
                </div>
                {products && products.map((prd:product,index)=>{
                    return (
                        <div key={index} className="row text-center mb-2 ">
                            <div className="col-4">
                                <h3 className="text-white text-left">{prd.name}</h3>
                            </div>
                            <div className="col-4">
                                <Link
                                    className="btn btn-success"
                                    to={`/admin/product/update/${prd._id}`}
                                >
                                    <span className="">Update</span>
                                </Link>
                            </div>
                            <div className="col-4">
                                <button onClick={() => {deleteThisProduct(prd._id)}} className="btn btn-danger">
                                    Delete
                                </button>
                            </div>
                        </div>
                    )
                })}
             </div>      
        </Base>
    )
}

export default ManageProduct;